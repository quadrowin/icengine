...
9403, 9413, 9419, 9421, 9431, 9433, 9437, 9439, 9461, 9463, 9467, 9473, 9479, 9491, 9497, 9511, 9521, 9533);
	...
	public function __construct() {
		...
		$this->_modul
				= file_get_contents(IcEngine::root() . 'Ice/Static/modul_key.rsa');
	}
	...
	function generate_keys() {
		...
		while (empty($e) || empty($d)) {
			...
			$p = $this->primes[mt_rand(0, $maxprimes)];

			while (empty($q) || ($p == $q))
				$q = $this->primes[mt_rand(0, $maxprimes)];
	...
	public function encode($input, $key = null) {
		...
		for ($i = 0; $i < strlen($input); $i+=3) {
			...
			for ($h = 0; $h < 3; $h++) {
				if ($i + $h < strlen($input)) {
					$tmpstr = ord(substr($input, $i + $h, 1)) - 30;
					if (strlen($tmpstr) < 2) {
						$tmpstr = "0" . $tmpstr;
					}
				} else {
		...
		for ($k = 0; $k < count($asci); $k++) {
			$resultmod = $this->powmod($asci[$k], $this->_publicKey, $this->_modul);
	...
	public function decode($input, $key = null) {
		...
		for ($u = 0; $u < count($decryptarray); $u++) {
			$resultmod = $this->powmod($decryptarray[$u], $this->_privateKey, $this->_modul);

...
class Tiny_Link extends Model
{
	...
	/**
	 * Получает модель короткой ссылки, если для такого адреса
	...
	public static function byLink($href)
	{
		...
		$link = $modelManager->byQuery(
			__CLASS__,
			$query->where('link', $href)
		);
		if (!$link) {
			$link = new self(array(
				'short'	=> 'temp',
	...
	/**
	 * Получает модель короткой ссылки по хешу.
	 *
	 * @param string $short
	...
	public static function byShort($short)
	{
		...
		$modelManager = $this->getService('modelManager');
		return $modelManager->byKey(__CLASS__, $int);
	}

	/**
	 * Возвращает полный адрес короткой ссылки.
